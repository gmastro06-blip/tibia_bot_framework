...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Interactively select an ROI and write it into the appropriate rois_guess config.")
    p.add_argument("--monitor", type=int, default=int(os.getenv("FORCE_MONITOR", "2") or 2))
    p.add_argument("--name", type=str, required=True, help="ROI name to write (e.g. cap_ocr, amulet_slot, ring_slot, soul_ocr)")
    p.add_argument(
        "--base",
        choices=["frame", "skills_panel", "equipment_slots", "right_hud_panel", "states_icons"],
        default="skills_panel",
        help="Pick ROI inside this base area.",
    )
    p.add_argument(
        "--force-tk",
        action="store_true",
    ...
    p.add_argument("--write", action="store_true", help="Write ROI into the config JSON.")
    return p.parse_args()
...
def main() -> int:
    ...
    def _select_roi(title: str, img_bgr: np.ndarray) -> tuple[int, int, int, int]:
        ...
        try:
            ...
            if rw <= 1 or rh <= 1:
                # Some OpenCV builds "succeed" but return empty ROI when GUI isn't actually functional.
                return _select_roi_tk(title, img_bgr)
            return rx, ry, rw, rh
        except Exception:
    ...
    rx, ry, rw, rh = _select_roi(f"Selecciona ROI: {args.name}", base_img)
    if rw <= 1 or rh <= 1:
    ...
    print(f"- {args.name} (norm): {json.dumps(roi_norm, ensure_ascii=False)}")
    ...
    if args.write:
        ...
        cfg["rois_guess_norm"][str(args.name)] = roi_norm
        with cfg_file.open("w", encoding="utf-8") as f:
            json.dump(cfg, f, ensure_ascii=False, indent=2)
        print(f"✍️  Escrito {args.name} en {cfg_path}")
...
def _select_roi_tk(title: str, img_bgr: "np.ndarray") -> tuple[int, int, int, int]:
    ...
    Returns (x, y, w, h) in the original image coordinates.
    """
    ...
    try:
        ...
        raise SystemExit(f"No se pudo importar cv2 para resize/cvtColor: {e}")
    ...
    # Scale down to fit a reasonable window.
    ...
    state: _DragState = {"x0": None, "y0": None, "x1": None, "y1": None, "rect": None, "done": False, "cancel": False}
