...
import time
import cv2
import dxcam  # Nuevo: ultra fast capture para RTX
import mss
import numpy as np
...
class CaptureProvider:
    def __init__(self, mode: str = "dxcam", window_title: str = "Tibia - Loterinne", resolution: Tuple[int, int] = (1920, 1080)):
        ...
        self.rect = (0, 0, resolution[0], resolution[1])  # Full window or screen
        self.fps = 0
        self.last_time = time.time()

        if self.mode == "dxcam":
            self.camera = dxcam.create(output_idx=0, output_color="BGR", max_buffer_len=64)  # RTX 4070 optima
    ...
    def capture(self) -> Optional[np.ndarray]:
        frame = None
        if self.mode == "dxcam":
            frame = self.camera.grab(region=self.rect)  # tuple (left, top, right, bottom)
            if frame is not None:
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # Si needed
        elif self.mode == "mss":
