...
class BattlelistExtractor:
    ...
    def segment_rows(self, battlelist_roi: np.ndarray) -> List[np.ndarray]:
        rows = []
    ...
    def extract(self, battlelist_roi: np.ndarray) -> List[Dict]:
    ...
    def measure_hp_bar(self, row: np.ndarray) -> float:
        hsv = cv2.cvtColor(row, cv2.COLOR_BGR2HSV)
        # Red hue detection (wraps around: 0-10 and 170-180)
        mask_red_low = cv2.inRange(
            hsv, np.array([0, 70, 50], dtype=np.uint8),
            np.array([10, 255, 255], dtype=np.uint8)
        )  # type: ignore[arg-type]
        mask_red_high = cv2.inRange(
            hsv, np.array([170, 70, 50], dtype=np.uint8),
