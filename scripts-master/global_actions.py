...
def waypoint_action(client, action):
    if action == "start":
    ...
    elif action == "loot_off":
        client.loot_on = False
        ...
        if not client.script_options.get('refill_from_stash', True):
            print('[Action] Refill from stash skipped')
            return
    ...
    elif action == "travel_goroma":
        client.npc_say(['goroma', 'yes'])
    ...
    elif action == "travel_roshamuul":
        client.npc_say(['roshamuul', 'yes'])
